...
class LiveWorkbookFormulaTests(unittest.TestCase):
    ...
    def test_parameters_customer_and_summary_match_approved_contract(self):
        workbook = openpyxl.Workbook()
        ...
        self.assertEqual(params["A1"].value, "Tham Số Chính Sách & Công Chuẩn")
        self.assertEqual(params["B2"].value, 3)
        self.assertEqual(params["B3"].value, 5)
        self.assertEqual(params["B4"].value, 30000)
        self.assertEqual(params["B5"].value, 10000)
        self.assertEqual(params["B8"].value, 50)
        self.assertEqual(params["B9"].value, 26)
        self.assertIsNone(params["A10"].value)

        facts = self._facts()
        customer_end = replace_customer_rows(customer, facts)
        self.assertEqual(customer_end, 5)
        self.assertEqual(customer["E5"].value, "Khách hàng cũ")
        self.assertEqual(customer["F5"].value, 2)
        self.assertIn("CHƯA ĐẠT", customer["I5"].value)
        self.assertEqual(customer["J5"].value, "ĐÃ GHI TỒN")
        self.assertIn("Khách hàng cũ", customer["P5"].value)
        self.assertIn("KHĐĐK", customer["Q5"].value)

        detail = pd.DataFrame([{"ma_kh": "KH001"}, {"ma_kh": "KH002"}])
        update_customer_image_formulas(customer, detail_count=2, detail_frame=detail)
        ...
        self.assertEqual(customer["R5"].value, "Dò ảnh")
        self.assertIsNotNone(customer["R5"].hyperlink)

        replace_summary_rows(summary, facts, customer_end)
        self.assertIn('"<>Không Đạt"', summary["D5"].value)
        self.assertIn("$B$8", summary["I5"].value)
        self.assertIn("$B$6", summary["M5"].value)
        self.assertIn("$B$7", summary["P5"].value)
        self.assertIn("Đạt xuất sắc", summary["Q5"].value)
    ...
    def test_empty_customer_frame_keeps_valid_table_contract(self):
        ...
        self.assertEqual(sheet.auto_filter.ref, "A4:R4")
        workbook.close()
